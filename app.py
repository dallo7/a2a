...
#     dcc.Interval(id='animation-interval', interval=1500, n_intervals=0, disabled=True),
#     dcc.Interval(id='interval-component', interval=3000, n_intervals=0, disabled=True),
#     dcc.Interval(id='notification-timer', interval=5000, n_intervals=0, disabled=True), # NEW: Timer for notification
...
#     # NEW: Popover for instant payment notifications
#     dbc.Popover(
#         [
#             dbc.PopoverHeader(
#                 [html.I(className="fas fa-check-circle text-success me-2"), "Payment Successful"],
#                 className="d-flex align-items-center"
#             ),
#             dbc.PopoverBody(id="payment-notification-body"),
#         ],
#         id="payment-notification-popover",
...
# # ==============================================================================
# # NEW CALLBACKS for the payment success notification popover
# # ==============================================================================

# @app.callback(
#     Output('payment-notification-popover', 'is_open'),
#     Output('payment-notification-body', 'children'),
#     Output('notification-timer', 'disabled'),
#     Input('transaction-refresh-signal', 'data'),
#     State('pending-transaction-store', 'data'),
#     prevent_initial_call=True
# )
# def display_payment_notification(signal, transaction_data):
#     """
#     This callback is triggered after a successful transaction.
#     It displays a popover with the transaction details.
#     """
#     if not signal or not transaction_data:
...
#     else:
#         # Don't show for unknown or failed transaction types
#         return False, no_update, True

#     notification_body = dbc.Alert(message, color="success")

#     # Open the popover and start the timer to close it automatically
#     return True, notification_body, False


# @app.callback(
#     Output('payment-notification-popover', 'is_open', allow_duplicate=True),
#     Output('notification-timer', 'disabled', allow_duplicate=True),
#     Input('notification-timer', 'n_intervals'),
#     prevent_initial_call=True
# )
# def auto_hide_notification(n):
#     """
#     This callback is triggered by the notification-timer interval.
#     It closes the popover and disables the timer.
#     """
#     return False, True
...
app.layout = dbc.Container([
    ...
    # FIX: Use a Toast for notifications instead of a Popover and remove the extra timer
    dbc.Toast(
        id="payment-toast-notification",
        header="Payment Successful",
        icon="success",  # FIX: Correct property is 'icon', not 'header_icon'
        is_open=False,
        dismissable=True,
        duration=6000,  # Auto-hides after 6 seconds
        style={"position": "fixed", "top": 20, "right": 10, "width": 350, "zIndex": 9999},
    ),

    # Dedicated store for IPN
...
def process_payment(pin, data):
    ...
    if pin != '1234':
        body = create_processing_animation(status='failed', message="Invalid PIN.")
        return True, body, False, '', no_update, True, True, 0, no_update
    ...
    if is_success:
        initial_body = create_processing_animation(step=0)
        return True, initial_body, False, '', datetime.datetime.now().isoformat(), True, False, 0, ipn_data
    else:
        fail_body = create_processing_animation(status='failed', message=msg)
        return True, fail_body, False, '', no_update, True, True, 0, no_update
