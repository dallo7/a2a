...
# # ==============================================================================
# # UPDATED CALLBACK: display_page now listens for refresh signals for the dashboard
# # ==============================================================================
# @app.callback(
#     Output('page-content', 'children'),
#     Input('url', 'pathname'),
#     Input('transaction-refresh-signal', 'data')  # FIX: Add this input
# )
# def display_page(pathname, refresh_signal):
#     triggered_id = callback_context.triggered_id
#     view = (pathname or "/").strip("/").split("/")[-1]

#     # Only refresh the dashboard view on a transaction signal
#     if view == 'dashboard' and (triggered_id == 'transaction-refresh-signal' or triggered_id == 'url'):
#         return create_dashboard()

#     # Standard navigation for all other cases
#     if triggered_id == 'url':
#         if view == 'payroll':
...
#             "SELECT a.bank, SUM(t.amount) as volume FROM transactions t JOIN accounts a ON t.to_acct = a.id WHERE t.status='completed' GROUP BY a.bank ORDER BY volume DESC LIMIT 5",
#             conn)
...
#     fig_scatter = px.scatter(df_trans_full, x='timestamp', y='amount', color=df_trans_full['type'].str.capitalize(),
...
# # ==============================================================================
# # UPDATED CALLBACK: update_animation_step now handles DB logic and triggers
# # ==============================================================================
# @app.callback(
...
#     prevent_initial_call=True
# )
# def update_animation_step(n, current_step, data):
#     if n == 0: return [no_update] * 6
...
#         return new_body, next_step, False, True, no_update, no_update

#     # Final step: Process transaction in DB and show final animation
#     is_success, msg = False, ""
#     try:
#         with get_db_connection() as conn:
#             # ... (Database logic is the same as the old process_payment)
#             if data['type'] == 'transfer':
#                 payload = data['payload']
#                 amount = payload['amountExpected']
#                 sender_acct = conn.execute("SELECT id, balance FROM accounts WHERE accountNumber = ?",
#                                            (payload['senderAccount'],)).fetchone()
#                 if not sender_acct: raise ValueError(f"Sender account {payload['senderAccount']} not found.")
#                 if sender_acct['balance'] < amount: raise ValueError("Insufficient funds.")
#                 receiver_acct = conn.execute("SELECT id FROM accounts WHERE accountNumber = ?",
...
#                 if bal < amount: raise ValueError("Insufficient corporate funds.")
...
#         # Disable animation interval, enable auto-close interval, send refresh signal, and send IPN signal
#         return final_body, next_step, True, False, datetime.datetime.now().isoformat(), data
#     else:
#         fail_body = create_processing_animation(status='failed', message=msg)
#         # Disable animation interval, enable auto-close interval, don't send signals
#         return fail_body, next_step, True, False, no_update, no_update
...
ENCODED_LOGO = encode_logo(LOGO_PATH)

# --- Initialize App ---
app = dash.Dash(
    __name__,
    suppress_callback_exceptions=True,
    # Use only BOOTSTRAP and FONT_AWESOME. Custom styles loaded via assets/
    external_stylesheets=[dbc.themes.BOOTSTRAP, dbc.icons.FONT_AWESOME, '/assets/capitalpay_brand.css'],
    meta_tags=[{"name": "viewport", "content": "width=device-width, initial-scale=1"}]
)

# *** IMPORTANT FIX: Removed app.index_string to fix InvalidIndexException ***
# The custom styling is now entirely handled by assets/capitalpay_brand.css
...
def create_header():
    ...
    ) if ENCODED_LOGO else html.I(className="fas fa-landmark",
                                  style={'color': CP_PRIMARY, 'fontSize': '2.5rem', 'marginRight': '20px'})
...
def create_footer():
    ...
    ) if ENCODED_LOGO else html.I(className="fas fa-landmark text-light me-2", style={'fontSize': '1rem'})
    ...
    for bank in sorted(display_banks):
        ...
        card = dbc.Col(dbc.Card([
            ...
            dbc.CardBody([
                ...
                html.Div([
                    html.P([html.I(className="fas fa-arrow-circle-down me-2", style={'color': CP_SECONDARY}),
                            "Transacted amount: ",
                            format_currency(metrics['inflow'])], className="mb-1 small"),
                    html.P([html.I(className="fas fa-arrow-circle-up me-2", style={'color': CP_PRIMARY}),
                            "Outgoing Cash Flow: ",
...
def handle_transfer_confirmation(
    ...
    with get_db_connection() as conn:
        ...
        sender_check = conn.execute("SELECT id, type, balance FROM accounts WHERE accountNumber = ?",
                                    (s_acct,)).fetchone()
        if not sender_check:
            return False, no_update, no_update, False, no_update, no_update, dbc.Alert(
...
def update_analytics_page(pathname, refresh_signal):
    ...
    fig_dist = px.pie(dist_data, names='range', values='count', title="Distribution of Transaction Sizes", hole=.3,
                      template=plot_template,
                      color_discrete_sequence=[CP_PRIMARY, CP_ACCENT_ORANGE, CP_SECONDARY, CP_ACCENT_PURPLE])
    ...
    for fig in [fig_payroll, fig_dist, fig_trends, fig_banks, fig_type_pie, fig_scatter]:
        fig.update_layout(title_x=0.5, margin=dict(l=20, r=20, t=40, b=20), plot_bgcolor='#ffffff',
